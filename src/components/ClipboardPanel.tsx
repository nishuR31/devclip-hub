...
import { Search, Plus, ClipboardPaste, Trash2, X, AlertCircle } from "lucide-react";
...
export function ClipboardPanel({
  entries, clipboardError, onReadClipboard, onPaste, onAddEntry, onDelete, onUpdate, onTogglePin, onCopy, onClearHistory,
}: Props) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-2 border-b p-3">
        ...
        <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">{entries.length}</span>
        <div className="ml-auto flex gap-1">
          <Button size="sm" variant="outline" className="h-7 gap-1 text-xs" onClick={onReadClipboard}>
            <ClipboardPaste className="h-3.5 w-3.5" /> Capture
          </Button>
          <Button size="sm" variant="outline" className="h-7 gap-1 text-xs" onClick={() => setShowAdd(!showAdd)}>
      ...
      {showAdd && (
        <div className="border-b p-3 animate-fade-in">
          <Textarea
            ...
            onPaste={handlePasteEvent}
          />
          <Button size="sm" onClick={handleAdd} className="h-7 text-xs">Add Entry</Button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">No clipboard entries</p>
            <p className="text-xs text-muted-foreground/60">Click Capture or paste content here</p>
          </div>
        ) : (
          filtered.map((entry) => (
            <ClipboardItem
      ...
      {entries.length > 0 && (
        <div className="border-t p-2">
          <Button size="sm" variant="ghost" className="h-7 gap-1 text-xs text-destructive hover:text-destructive" onClick={onClearHistory}>
