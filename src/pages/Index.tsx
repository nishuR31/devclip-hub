...
import {
  Clipboard, Code2, Database, Sun, Moon, PanelLeftClose, PanelRightClose,
  Shield, PanelLeft, PanelRight, Home, ArrowLeft, User, CreditCard, LogOut, Settings,
} from "lucide-react";
...
const Index = () => {
  ...
  const [mobileView, setMobileView] = useState<"main" | "clipboard" | "snippets" | "inspector">("main");
    ...
    return (
      <div className="flex h-[100dvh] flex-col bg-background text-foreground">
        ...
        <header className="flex h-12 shrink-0 items-center gap-2 border-b px-3">
          {mobileView !== "main" && (
            <Button size="icon" variant="ghost" className="h-8 w-8 shrink-0" onClick={() => setMobileView("main")}>
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h1 className="text-sm font-bold tracking-tight truncate">{mobileTitle[mobileView]}</h1>
          </div>
          ...
          {mobileView === "main" && (
            <div className="p-4 space-y-4">
              <div className="rounded-xl border bg-card p-6 text-center">
                ...
                <h2 className="mb-1.5 text-xl font-bold tracking-tight">DevClipboard Hub</h2>
                <p className="text-xs text-muted-foreground">
                  Clipboard manager & browser data inspector
                </p>
                <div className="mt-4 flex flex-col gap-2">
                  <Button onClick={clipboard.readClipboard} className="gap-2 w-full">
                    <Clipboard className="h-4 w-4" /> Read Clipboard
                  </Button>
                  <div className="grid grid-cols-2 gap-2">
                    <Button variant="outline" onClick={() => setMobileView("clipboard")} className="gap-1.5 text-xs">
                      <Clipboard className="h-3.5 w-3.5" /> History
                    </Button>
                    <Button variant="outline" onClick={() => setMobileView("snippets")} className="gap-1.5 text-xs">
                      <Code2 className="h-3.5 w-3.5" /> Snippets
                    </Button>
                  </div>
                  <Button variant="outline" onClick={() => { setMobileView("inspector"); browserData.refresh(); }} className="gap-2 w-full">
              ...
              {clipboard.currentClipboard && (
                <div className="rounded-xl border bg-card p-4 animate-fade-in">
                  <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Current Clipboard</h3>
              ...
              {clipboard.clipboardError && (
                <div className="rounded-lg border border-warning/30 bg-warning/5 p-3">
                  <p className="text-xs text-warning">{clipboard.clipboardError}</p>
                </div>
              )}

              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: "Clipboard", value: clipboard.entries.length, icon: Clipboard, tap: () => setMobileView("clipboard") },
                  { label: "Snippets", value: snippets.snippets.length, icon: Code2, tap: () => setMobileView("snippets") },
                  { label: "Storage", value: browserData.localStorageItems.length + browserData.sessionStorageItems.length, icon: Database, tap: () => { setMobileView("inspector"); browserData.refresh(); } },
                ].map((stat) => (
                  <button key={stat.label} onClick={stat.tap} className="rounded-lg border bg-card p-3 text-center transition-colors hover:border-primary/30 active:bg-muted">
              ...
              <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 text-center">
                <div className="flex items-center justify-center gap-1.5 mb-0.5">
                  ...
                  <span className="text-[10px] font-semibold text-primary">Cloud Synced</span>
                </div>
              </div>
            </div>
          )}
        ...
        <nav className="flex h-14 shrink-0 items-center justify-around border-t bg-card safe-area-bottom">
          {([
            ...
            <button
              key={item.key}
              onClick={() => { setMobileView(item.key); if (item.key === "inspector") browserData.refresh(); }}
              className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-lg transition-colors active:scale-95 ${
                mobileView === item.key ? "text-primary" : "text-muted-foreground"
              }`}
            >
              <item.icon className={`h-5 w-5 ${mobileView === item.key ? "text-primary" : ""}`} />
    ...
    <div className="flex h-screen flex-col bg-background text-foreground">
      <header className="flex h-11 shrink-0 items-center gap-3 border-b px-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-bold tracking-tight hidden sm:block">DevClipboard Hub</h1>
        </div>
        <div className="ml-auto flex items-center gap-1">
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setLeftOpen(!leftOpen)}>
            {leftOpen ? <PanelLeftClose className="h-3.5 w-3.5" /> : <PanelLeft className="h-3.5 w-3.5" />}
          </Button>
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setRightOpen(!rightOpen)}>
            {rightOpen ? <PanelRightClose className="h-3.5 w-3.5" /> : <PanelRight className="h-3.5 w-3.5" />}
          </Button>
          <div className="mx-1 h-4 w-px bg-border" />
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={toggleTheme}>
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-6">
          <div className="mx-auto max-w-3xl space-y-4 lg:space-y-6">
            <div className="rounded-xl border bg-card p-6 lg:p-8 text-center">
              ...
              <h2 className="mb-2 text-xl lg:text-2xl font-bold tracking-tight">DevClipboard Hub</h2>
              <p className="text-xs lg:text-sm text-muted-foreground max-w-md mx-auto">
                ...
                Capture clipboard content, manage snippets, and inspect browser storage — all in one place.
              </p>
              <div className="mt-4 lg:mt-6 flex flex-wrap justify-center gap-2">
                <Button onClick={clipboard.readClipboard} className="gap-2">
                  <Clipboard className="h-4 w-4" /> Read Clipboard
                </Button>
                <Button variant="outline" onClick={() => { setLeftOpen(true); setLeftTab("snippets"); }} className="gap-2">
                  <Code2 className="h-4 w-4" /> Manage Snippets
                </Button>
                <Button variant="outline" onClick={() => { setRightOpen(true); browserData.refresh(); }} className="gap-2">
                  <Database className="h-4 w-4" /> Inspect Storage
                </Button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 lg:gap-3">
              {[
                { label: "Clipboard Entries", value: clipboard.entries.length, icon: Clipboard },
                { label: "Snippets", value: snippets.snippets.length, icon: Code2 },
                { label: "Storage Keys", value: browserData.localStorageItems.length + browserData.sessionStorageItems.length, icon: Database },
              ].map((stat) => (
                <div key={stat.label} className="rounded-lg border bg-card p-3 lg:p-4 text-center">
            ...
            <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 text-center">
              <div className="flex items-center justify-center gap-2 mb-1">
                ...
                <span className="text-xs font-semibold text-primary">Cloud Synced</span>
              </div>
