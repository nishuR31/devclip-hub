...
export const workspaceRepository = {
  ...
  async clearUnpinnedClipboard(userId: string, plan: PlanTier): Promise<void> {
    if (plan === "FREE") return;
    await prisma.$runCommandRaw({
      delete: collectionFor("clipboard"),
  ...
  async getTeamMembers(userId: string, plan: PlanTier): Promise<string[]> {
    if (plan !== "TEAM") return [];
